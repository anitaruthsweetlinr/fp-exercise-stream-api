...
public class StreamExercise {
    ...
    public void task9() {
        ...
        List<PersonDto> dtoList = people.stream().filter(person -> person.getDateOfBirth().isBefore(LocalDate.parse("1920-01-01"))).map(convertPersonToDto).collect(Collectors.toList());
    ...
    public void task10() {
        ...
        Optional<String> optional = people.stream().filter(person -> person.getPersonId()== 5914).
                map(person -> person.getDateOfBirth().getDayOfWeek().toString() + " " + person.getDateOfBirth().getDayOfMonth()
                        + " "+ person.getDateOfBirth().getMonth().toString() + " " + person.getDateOfBirth().getYear()).findFirst();
        assertNotNull(optional);
        assertTrue(optional.isPresent());
        assertEquals(expected, optional.get());
    }
    ...
    public void task11() {
        ...
        double averageAge = people.stream().mapToInt(personToAge).average().orElse(0.0);
    ...
    public void task12() {
        ...
        Predicate<Person> checkIFPalindrome = person -> {
            String reversedFName = new StringBuilder(person.getFirstName()).reverse().toString();
            return person.getFirstName().equalsIgnoreCase(reversedFName);
        };
        String[] result = people.stream().filter(checkIFPalindrome)
    ...
    @Test
    public void task14() {
        LocalDate[] _2020_dates = Stream.iterate(LocalDate.of(2020,1, 1 ) ,date -> date.plusDays(1)).limit(366).toArray(LocalDate[]::new);
